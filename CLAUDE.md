...
## Where things live
- `docs/architecture.md` — connected picture; `docs/contracts.md` — Envelope, payloads, state machines; `docs/traceability.md` — LLM observability
...
- Stories: end-user perspective first; bugs filed as cards immediately; eval-gated stories need a result.
- Secrets only in `.env` (template `.env.example`) — never in code, workflows, or exports; `.mcp.json` loads via dotenv-cli.

## Gotchas
- Old v1 artifacts: reference only. n8n holds no state between executions — persist to Postgres if it matters.
...
- n8n 2.x API: PUT edits the DRAFT only, rejects off-schema settings (`binaryMode`) — whitelist; Code-node errors only in `stack`; waiting executions invisible.
- Exports drift: UI edits strip defaults, move nodes — re-export before zipping.
...
- No build work without a story card; one at a time.
- No PII in prompts beyond fields a step needs; none in chart URLs. Never tune eval labels to match output.
